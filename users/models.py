...
class UserManager(BaseUserManager):
    ...
    def _create_user(self, username, phone_number, email, password,is_staff, is_superuser,
                      **extra_fields):
...
class User(AbstractBaseUser, PermissionsMixin):
    username = models.CharField(_('username'), max_length=32, unique=True,
                                help_text=
                                    _('Required, 32 characters or fewer. starting with a Letters, Letters, digits and hyphens. '),
                                validators=[
                                    validators.RegexValidator(r'^[a-zA-Z][a-zA-Z0-9_\.]+$',
                                                              _('Enter a valid username. starting with a-z'), 'invalid'),
                                ],
                                error_messages={
...
class UserProfile(models.Model):
    ...
    avatar = models.ImageField(_('avatar'), upload_to='users/%Y/%m', blank=True)
    birthday = models.DateField(_('birthday'), null=True, blank=True)
    gender = models.BooleanField(_('gender'), help_text='female is False, male is True, Null is Unset', null=True, blank=True)
